...
public class KakaoMoNewAccountScheduler {
    ...
    public void detectAndInit() {
        List<Document> accounts = accountMongo.findKakaoMoAccounts();
        Set<String> seen = new HashSet<>();

        for (Document acc : accounts) {
            String userId = acc.getString("user_id");
            String advkey = acc.getString("account_kakaomoment");
            ...
            try {
                ...
                log.error("[NEW-ACCOUNT][KAKAO-MO] 초기 수집 실패 userId={} advkey={} error={}", userId, advkey, e.getMessage(), e);
                initiated.add(advkey);
            } finally {
