...
public class KakaoSaTokenManager {
    ...
    public void autoRefresh() {
        ...
        if (!isTokenValid()) {
            log.info("[KAKAO-SA TOKEN] 만료 감지 → 갱신 시작");
            doRefresh();
        }
    }
    ...
    private boolean isTokenValid() {
        try {
            ...
            log.warn("[KAKAO-SA TOKEN] 토큰 만료 (401)");
            return false;
        } catch (Exception e) {
            log.warn("[KAKAO-SA TOKEN] verify 실패: {}", e.getMessage());
            return false;
        }
    }
    ...
    private void doRefresh() {
        try {
            ...
            if (resBody == null || resBody.get("access_token") == null) {
                log.error("[KAKAO-SA TOKEN] 갱신 응답 이상: {}", resBody);
                return;
            }
            ...
            log.error("[KAKAO-SA TOKEN] 갱신 실패: {}", e.getMessage());
        }
    }
}
