...
public class NaverAdGroupDayCollectionJob {
    ...
    private void collectAccount(NaverAccountDto acc, String fromDate, String toDate) {
        ...
        for (String date : dates) {
            int saved = collectDay(client, customerId, adGroups, date);
            log.info("[NaverAdGroupDay] customerId={} date={} saved={}", customerId, date, saved);
        }
        accountLogMongo.updateField(customerId, "naver", "adgroup");
    }
    ...
    private int collectDay(NaverApiClient client, String customerId, List<NaverAdGroupDto> adGroups, String date) {
        ...
        for (NaverAdGroupDto ag : adGroups) {
            ...
            row.put("campaign_id", ag.getCampaignId());
            row.put("adgroup_id",  ag.getAdgroupId());
    ...
    private void fetchStats(NaverApiClient client, List<String> ids, Map<String, long[]> resultMap, String date) {
        ...
        try {
            ...
            log.error("[NaverAdGroupDay] stats 조회 실패 date={} error={}", date, e.getMessage());
            return;
        }
