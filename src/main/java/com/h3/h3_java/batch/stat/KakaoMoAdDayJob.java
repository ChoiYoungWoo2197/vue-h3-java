...
public class KakaoMoAdDayJob {
    ...
    private void collectDate(KakaoMoApiClient api, String advkey, String date,
        ...
        for (Map<String, Object> ag : adgroups) {
            ...
            // 100개씩 배치 처리
            for (int i = 0; i < adIds.size(); i += BATCH_SIZE) {
                if (cnt >= MAX_PER_DAY) break;

                List<String> batch = adIds.subList(i, Math.min(i + BATCH_SIZE, adIds.size()));
                ...
                for (Map.Entry<String, long[]> entry : agg.entrySet()) {
                    ...
                    doc.put("campaign_id", aidToCid.getOrDefault(adId, cid));
                    doc.put("adgroup_id", aidToGid.getOrDefault(adId, gid));
                    doc.put("ad_id",      adId);
