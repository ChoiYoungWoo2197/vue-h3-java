...
/**
 * 구글 소재 일별 수집.
 * PHP googleaddaycollection.php → MongoDB google_ad_daily.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class GoogleAdDayJob {
    ...
    public void collect() {
        ...
        log.info("[GOOGLE][AD-DAY] 전체 수집 시작 accounts={}", accounts.size());
        String token = tokenManager.getAccessToken();
        if (token == null) { log.warn("[GOOGLE][AD-DAY] 토큰 없음"); return; }
    ...
    private void collectForAccount(GoogleAccountDto account, List<String> dates, String token) {
        ...
        if (ads.isEmpty()) { log.debug("[GOOGLE][AD-DAY] 소재 없음 advkey={}", advkey); return; }

        Map<String, Map<String, String>> aidToIds = new HashMap<>();
        for (Map<String, Object> ad : ads) {
            String aid = str(ad, "aid");
            if (aid == null) continue;
            Map<String, String> ids = new HashMap<>();
            ids.put("campaign_id", str(ad, "campaign_id"));
            ids.put("adgroup_id",  str(ad, "adgroup_id"));
            aidToIds.put(aid, ids);
        }

        for (String date : dates) {
            if (LocalDate.parse(date, FMT).compareTo(LocalDate.now()) >= 0) continue;

            String query = "SELECT campaign.id, ad_group.id, ad_group_ad.ad.id, " +
                "metrics.clicks, metrics.conversions, metrics.cost_micros, " +
                "metrics.impressions, metrics.conversions_value " +
                "FROM ad_group_ad WHERE segments.date = '" + date + "'";
            ...
            for (Map<String, Object> row : rows) {
                ...
                if (aga == null || m == null) continue;
                Map<String, Object> ad = (Map<String, Object>) aga.get("ad");
                if (ad == null) continue;
                String aid = str(ad, "id");
                if (aid == null || !aidToIds.containsKey(aid)) continue;

                Map<String, String> ids = aidToIds.get(aid);
                String cid = c != null ? str(c, "id") : ids.get("campaign_id");
                String gid = g != null ? str(g, "id") : ids.get("adgroup_id");
    ...
    private List<String> buildAutoDates(String advkey) {
        ...
        for (int i = 1; i <= 7; i++) {
            ...
            if (!statMongoService.hasAdDaily(advkey, d)) dates.add(d);
        }
