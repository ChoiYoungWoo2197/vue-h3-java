...
public class NaverGfaBudgetAlarmJob {
    ...
    private void accountBudgetAlarm(String userId, String advkey, Document alarmSetting) {
        for (String kpi : KPIS) {
            String periodStr = getStr(alarmSetting.get(kpi));
            if (periodStr.length() < 2) continue;

            if (budgetAlarmMongo.countRecentAlarm(userId, advkey, "naverda", "account", null,
    ...
    private void campaignBudgetAlarm(String userId, String advkey, Document alarmSetting) {
        ...
        for (Map<String, Object> campaign : campaigns) {
            ...
            if (toInt(campaign.get("onoff")) == 0) continue;

            for (String kpi : KPIS) {
                String periodStr = getStr(alarmSetting.get(kpi));
                if (periodStr.length() < 2) continue;

                if (budgetAlarmMongo.countRecentAlarm(userId, advkey, "naverda", "campaign",
