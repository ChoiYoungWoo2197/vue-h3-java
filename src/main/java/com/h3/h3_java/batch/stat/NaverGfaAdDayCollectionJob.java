...
public class NaverGfaAdDayCollectionJob {
    ...
    private void collectAccount(NaverGfaAccountDto acc, String fromDate, String toDate) {
        ...
        if (token == null || managerNo == null) {
            log.warn("[GFA][AD-DAY][SKIP] 토큰 없음 userId={}", acc.getUserId());
            return;
        }

        // aid → gid 매핑
        Map<String, String> adMap = mongoService.selectGfaAds(advkey);
        if (adMap.isEmpty()) {
            log.warn("[GFA][AD-DAY] 소재 없음 advkey={}", advkey);
            return;
        }
        ...
        if (fromDate != null && toDate != null) {
            for (DateRange chunk : buildChunkedRanges(fromDate, toDate)) {
                if (!collectChunk(advkey, token, managerNo, adMap, adgroupMap, chunk.from(), chunk.to())) break;
                try { Thread.sleep(1000 + new Random().nextInt(400)); }
                catch (InterruptedException e) { Thread.currentThread().interrupt(); return; }
            }
        } else {
            for (String date : buildAutoDates(advkey)) {
                if (!collectDay(advkey, token, managerNo, adMap, adgroupMap, date)) break;
                try { Thread.sleep(1000 + new Random().nextInt(400)); }
                catch (InterruptedException e) { Thread.currentThread().interrupt(); return; }
            }
        }
        accountLogMongo.updateField(advkey, "naverda", "ad");
    }

    @SuppressWarnings("unchecked")
    private boolean collectChunk(String advkey, String token, String managerNo,
                                 Map<String, String> adMap, Map<String, String> adgroupMap,
                                 String fromDate, String toDate) {
        String baseUrl = GFA_DAILY_BASE + "/adAccounts/" + advkey
            + "/performance/past/creatives?startDate=" + fromDate + "&endDate=" + toDate + "&limit=1000";
        ...
        do {
            ...
            try {
                ...
                if (e.getStatusCode().value() == 403) {
                    log.warn("[GFA][AD-DAY] 접근 권한 없음 advkey={} → 계정 스킵", advkey);
                    return false;
                }
                log.error("[GFA][AD-DAY] API 오류 advkey={} from={} to={} error={}", advkey, fromDate, toDate, e.getMessage());
                break;
            } catch (Exception e) {
                log.error("[GFA][AD-DAY] API 오류 advkey={} from={} to={} error={}", advkey, fromDate, toDate, e.getMessage());
                break;
            ...
            if (rows != null) {
                for (Map<String, Object> r : rows) {
                    String creativeNo = String.valueOf(r.getOrDefault("creativeNo", ""));
                    if (!adMap.containsKey(creativeNo)) continue;

                    String date = toDateStr(r);
                    if (date == null) {
                        log.warn("[GFA][AD-DAY] chunk 응답 date 필드 없음 advkey={} rowKeys={}", advkey, r.keySet());
                        continue;
                    }
                    ...
                    String gid = adMap.get(creativeNo);
                    String cid = adgroupMap.getOrDefault(gid, "");

                    Map<String, Object> row = new LinkedHashMap<>();
                    row.put("daily_dt",    date);
                    row.put("daily_advid", advkey);
                    row.put("campaign_id", cid);
                    row.put("adgroup_id",  gid);
                    row.put("ad_id",       creativeNo);
                    row.put("daily_im",    im);
                    ...
                    mongoService.upsertGfaAdDaily(row);
                    saved++;
                }
            }
        ...
        log.info("[GFA][AD-DAY] chunk advkey={} from={} to={} saved={}", advkey, fromDate, toDate, saved);
        return true;
    }

    @SuppressWarnings("unchecked")
    private boolean collectDay(String advkey, String token, String managerNo,
                               Map<String, String> adMap, Map<String, String> adgroupMap, String date) {
        if (!LocalDate.parse(date, DATE_FMT).isBefore(LocalDate.now())) return true;

        String baseUrl = GFA_DAILY_BASE + "/adAccounts/" + advkey
            + "/performance/past/creatives?startDate=" + date + "&endDate=" + date + "&limit=1000";
        ...
        do {
            ...
            try {
                ...
                log.error("[GFA][AD-DAY] API 오류 advkey={} date={} error={}", advkey, date, e.getMessage());
                break;
            } catch (Exception e) {
                log.error("[GFA][AD-DAY] API 오류 advkey={} date={} error={}", advkey, date, e.getMessage());
                break;
        ...
        log.info("[GFA][AD-DAY] advkey={} date={} saved={}", advkey, date, saved);
        return true;
    }
    ...
    private List<String> buildAutoDates(String advkey) {
        ...
        for (int i = 1; i <= 7; i++) {
            ...
            if (!mongoService.hasGfaAdDailyData(advkey, d)) {
                dates.add(d);
            }
        }
